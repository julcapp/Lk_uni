...
async function main() {
  ...
  try {
    for (const path of ['/login/', '/create-project/', '/workspace/', '/profile/', '/organization/', '/forgot-password/', '/reset-password/']) {
    ...
    const created = await request(app)
      ...
      .set('x-device-name', 'Acceptance smoke')
      .send({
        projectName: 'Acceptance Project',
        owner: { name: 'Первый пользователь', email, password: 'acceptance-password-123' },
      })
      .expect(201);
    ...
    const profile = await request(app)
      ...
      .send({ displayName: 'Первый пользователь', phone: '+7 900 111-22-33' })
      .expect(200);
    ...
    const settings = await request(app)
      ...
      .set('authorization', `Bearer ${accessToken}`)
      .send({ language: 'ru', timezone: 'Europe/Moscow' })
      .expect(200);
    assert.equal(settings.body.language, 'ru');
    ...
    const organization = await request(app)
      ...
      .send({
        ...
        description: 'Приёмочное испытание',
    ...
    const confirmed = await request(app)
      ...
      .send({ token, password: 'acceptance-password-456' })
      .expect(200);
    ...
    await request(app)
      ...
      .send({ email, password: 'acceptance-password-123' })
      .expect(401);

    const loggedIn = await request(app)
      .post('/api/v1/auth/password-login')
      .send({ email, password: 'acceptance-password-456' })
      .expect(200);
    ...
    const finalOrganization = await request(app)
      .get('/api/v1/organization')
      .set('authorization', `Bearer ${loggedIn.body.tokens.accessToken}`)
      .expect(200);
    assert.equal(finalOrganization.body.organization.name, 'Acceptance Organization');

    console.log('Acceptance smoke test passed: UI routes, workspace, profile, organization and password recovery');
  } finally {
    await db.destroy();
  }
}
