...
## 5.1. Контуры доступа ChatGPT App

Доступ разделяется на три изолированных контура:

1. `User App` — только собственный профиль, identities и сессии пользователя.
2. `Project Admin App` — управление только одним разрешённым tenant после проверки административной роли.
3. `Platform Owner Console` — глобальные политики платформы; существует отдельно от пользовательского ChatGPT App.

Первый ChatGPT App работает в режиме `User App` и преимущественно только читает данные. Концептуальные, архитектурные и глобальные настройки не регистрируются как пользовательские MCP-инструменты.

Проверка прав выполняется backend Lk_uni для каждого вызова. Модель и интерфейс не являются источником решений об авторизации.
